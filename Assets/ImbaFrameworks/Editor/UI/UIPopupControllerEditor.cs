...
//            GUILayout.EndHorizontal();
            ...
            if (GUI.changed)
            {
               ...
               // EditorSceneManager.MarkSceneDirty(PopupController.scene);
            }
...
//        }
        ...
        {
            UIPopup popup2 = LoadPopup();
            if(popup2)
                UIAnimatorUtils.PreviewPopupAnimation(popup2, anim);
            
            UIPopupController controller = target as UIPopupController;
            if (controller != null)
            {
                UIPopup popup = controller.GetComponent<UIPopup>();
        ...
        private void StopPreviewAnim(bool destroyPreviewer = true)
        {
            ...
            if (controller != null)
            {
                ...
                if(popup)
                    UIAnimatorUtils.StopPreview(popup);
            }
           ...
           // string location = controller.CustomResourcesLocation + UIPopupManager.POPUP_PREFAB_LOCATION;
            ...
            if (zLoader)
            {
                ...
                if (!dialogInstance)
                {
                    ...
                    GameObject newInstance = controller.gameObject;//AssetDatabase.LoadAssetAtPath(prefabPath, typeof(GameObject)) as GameObject;
                    if (newInstance)
                    {
                        dialogInstance = PrefabUtility.InstantiatePrefab(newInstance) as GameObject;
                        dialogInstance.gameObject.name = controller.name;
                        dialogInstance.transform.SetParent(zLoader.transform, false);
                    }
                    else
                    {
                        Debug.LogError("Not found " + controller.name);
                    }
                }
                else
                {
        ...
        private GameObject FindPopup()
        {
            UIPopupController controller = target as UIPopupController;
            GameObject zLoader = UILoader();
            Transform tmp = zLoader.transform.Find(controller.name);
