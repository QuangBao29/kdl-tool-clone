    ...
    public class AssetBundleManager : IDisposable
    {
        ...
        public AssetBundleManager SetBaseUri(string[] uris)
        {
            if (baseUri == null || baseUri.Length == 0) {
                Debug.Log("Setting base uri to [{0}]." + string.Join(",", uris));
            } else {
                Debug.LogWarning("Overriding base uri from [{0}] to [{1}]." + string.Join(",", baseUri) +  string.Join(",", uris));
            }
        ...
        private void GetManifestInternal(string bundleName, uint version, int uriIndex)
        {
            ...
            handler.Handle(new AssetBundleDownloadCommand {
                ...
                OnComplete = manifest => {
                    ...
                    {
                        Debug.Log("Unable to download manifest from [{0}], attempting [{1}]" + baseUri[uriIndex] + baseUri[uriIndex + 1]);
                    ...
                    //    Debug.Log("Unable to download manifest, attempting to use one previously downloaded (version [{0}])." + version);
        ...
        public string GetHashedBundleName(string bundleName)
        {
            try {
                ...
                Debug.LogWarning("Unable to find hash for bundle [{0}], this request is likely to fail." + bundleName);
            }
