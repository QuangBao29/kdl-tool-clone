...
namespace KAP.Tools
{
    public class ConfigAllThemeInRoom : Editor
    {
        const string _configDecoFilePath = "/_KAP/_GameResources/Configs/Deco/ConfigDeco.csv";
        const string _configRoomFilePath = "/_KAP/_GameResources/Configs/Room/ConfigRoom.csv";
        const string _jsonRoomFolderPath = "/_KAP/_GameResources/Maps/Rooms/";

        [MenuItem("Tools/KAP/Config Theme Use In Room", false, 2)]
        public static void ConfigAllThemeUseInRoom()
        {
            ...
            var txtConfigRoom = FileSaving.Load(Application.dataPath + _configRoomFilePath);
            var configRoom = new ConfigRoom();
            configRoom.LoadFromString(txtConfigRoom);

            foreach (var record in configRoom.Records)
            {
                GetThemeUseInRoom(record, configDeco);
            }
        ...
        static void GetThemeUseInRoom(ConfigRoomRecord record, ConfigDeco configDeco)
        {
            var jsonPath = Application.dataPath + _jsonRoomFolderPath + record.Id + ".json";
