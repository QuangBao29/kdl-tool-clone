...
namespace OneP.Samples
{
	...
	{
		public Dictionary<string, UnityWebRequest> GetLoader(){
			return dicResourceLoader;
		} 
		private Dictionary<string, UnityWebRequest> dicResourceLoader = new Dictionary<string, UnityWebRequest> ();
		...
		public void DownLoadTexture(string url,Action<Texture2D> callback=null){
			...
			if (dicTextureLoader.ContainsKey (url)) {
                ...
                UnityWebRequest w = GetWWW (url);
				if (w != null) {
                    var texture = DownloadHandlerTexture.GetContent(w);
                    if (texture != null) {
						dicTextureLoader [url] = texture;
						if (callback != null) {
							callback (texture);
						}
					}
                    ...
                    DownloadFileUtil.Instance.OnDownloadFile(url,
                                                              wwwCallBack => {
                                                                  ...
                                                                  {
                                                                      var webRequest = wwwCallBack.www;
                                                                      if (webRequest != null)
                                                                      {
		...
		public void AddWWW(string key, UnityWebRequest w)
		{
			if (w != null && string.IsNullOrEmpty (w.error)) {
		...
		public void DisposeWWW(string url,UnityWebRequest w)// using to free memory
		{
			try
			{
				if(w!=null)
	...
	public class ExtraInfoDownload{
		...
		public ExtraInfoDownload(UnityWebRequest _www,float _timeDownload,int _retryTime){
			this.www = _www;
		...
		private IEnumerator OnDownloadFileRoutine(string url,Action<float> processCallback,float secondTimeout,int maxRetry, int retryCount=0)
		{
			...
			if (w!=null) {// kiem tra co cache hay khong
                ...
                using (w)
                {
                    ...
                    while (!w.isDone)
                    {
                        if (Mathf.Abs(cachePercent - w.downloadProgress) > 0.0f)
                        {
                            cachePercent = w.downloadProgress;
                            unDownLoadStep = 0;
                            if (processCallback != null)
                            {
